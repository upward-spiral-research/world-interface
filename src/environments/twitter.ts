...
function formatTweet(tweet: Tweet): string {
  ...
  }): ${tweetText} // Replies: ${metrics.reply_count}, Likes: ${
    metrics.like_count
  }, Retweets: ${metrics.retweet_count}, Views: ${metrics.impression_count}\n`;
}
...
export class Twitter {
  ...
  constructor() {
    this.baseUrl = process.env.TWITTER_PROXY_BASE_URL || '';
  ...
  async home(): Promise<TwitterResponse> {
    try {
      const response = await axios.get(`${this.baseUrl}api/get_home_timeline`, {
        headers: { Authorization: `Bearer ${this.apiKey}` },
      });
      const homeTweets = response.data.map((tweet: Tweet) => formatTweet(tweet)).join('\n');
      return {
  ...
  async post(commandString: string): Promise<TwitterResponse> {
    try {
      const { text, replyTo, mediaUrl } = this.parsePostCommand(commandString);
      ...
      return {
        title:
          "Your tweet was posted successfully to your personal Twitter account. Use 'twitter timeline' to see your recent posts",
  ...
  async unretweet(sourceTweetID: string): Promise<TwitterResponse> {
    try {
      const response = await axios.post(
  ...
  async profile(): Promise<TwitterResponse> {
    try {
      ...
      return {
        title:
          "These are recent tweets which you, personally, have posted to your Twitter account. Use 'twitter get <tweet_id>' to see a particular tweet's replies and conversation thread. You could also use 'twitter mentions' to see your other mentions",
        content:
          tweets ||
  ...
  async getMentions(): Promise<TwitterResponse> {
    try {
      const response = await axios.get(`${this.baseUrl}api/pull_mentions`, {
        headers: { Authorization: `Bearer ${this.apiKey}` },
      });
      const mentions = response.data.mentions.map((tweet: Tweet) => formatTweet(tweet)).join('\n');
      return {
  ...
  async getTweet(tweetId: string): Promise<TwitterResponse> {
    try {
      ...
      const { ancestor_chain, requested_tweet, sibling_tweets, children_tweets } =
        response.data as {
          ancestor_chain: Tweet[];
          requested_tweet: Tweet;
          sibling_tweets: Tweet[];
          children_tweets: Tweet[];
        };
      ...
      if (ancestor_chain && ancestor_chain.length > 0) {
        content += ancestor_chain.map((tweet: Tweet) => formatTweet(tweet)).join('\n');
      }
      ...
      if (sibling_tweets && sibling_tweets.length > 0 && !children_tweets.length) {
        content += sibling_tweets.map((tweet) => formatTweet(tweet)).join('\n');
      }

      // Add children
      if (children_tweets && children_tweets.length > 0) {
        content += children_tweets.map((tweet) => formatTweet(tweet)).join('\n');
      }
